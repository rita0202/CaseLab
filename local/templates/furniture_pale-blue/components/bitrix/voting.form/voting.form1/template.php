...
if (!empty($arResult["OK_MESSAGE"])): 
	?>
	<div class="vote-note-modal vote-note-note">
	<div class="close-modal" onclick="closeModalWindow()">+</div>
	<div class="vote-note-modal-text">Ваш голос записан.<br>Спасибо за участие в опросе!</div>
	</div>
	<?
endif;
...
?>	
	<li class="vote-item-vote <?=($iCount == 1 ? "vote-item-vote-first " : "")?><?
		...
		<div class="vote-item-header" style="display: <?=($iCount == 1 ? "unset" : "none")?>">
...
?>
	<div class="vote-item-title vote-item-question"><?=$arQuestion["QUESTION"]?><?if($arQuestion["REQUIRED"]=="Y"){echo "<span class='starrequired'>*</span>";}?></div>
		...
		<ol class="vote-items-list vote-answers-list" style="display: <?=($iCount == 1 ? "unset" : "none")?>">
<?
		$iCountAnswers = 0;
...
?>
			<span class="questions-count">
				<p>Вопрос <?=$iCount?> из <?=count($arResult["QUESTIONS"])?></p>
			</span>
			<div class="vote-form-item-buttons">
				<div>
					<span class="vote-form-item-buttons vote-form-item-buttons-prev" onclick="prevItem(<?=$iCount?>)" style="display: 
					<?=($iCount == 1 ? "none" : "unset")?>"><?=GetMessage("PREV_QUESTION")?></span>
				</div>
				<div>
					<span class="vote-form-item-buttons vote-form-item-buttons-next" onclick="nextItem(<?=$iCount?>)" style="display: 
					<?=($iCount == count($arResult["QUESTIONS"]) ? "none" : "unset")?>"><?=GetMessage("NEXT_QUESTION")?></span>
				</div>
			</div>
		</ol>
	</li>
...
<script src="../local/templates/furniture_pale-blue/components/bitrix/voting.form/voting.form1/index.js"></script>
